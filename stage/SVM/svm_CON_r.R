...
plot(x, pch = 17, col=y+3, cex=1)   ##mi da rosso o blu in base a se in y c'e' 1 o -1
...
##Dite a SVM che il kernel è lineare, il costo del parametro di sintonizzazione è 1 e la scala è uguale a falsa. 
##In questo esempio, viene richiesto di non standardizzare le variabili.

##Plot data -> svm fa i conti qua
...
make.grid = function(x, n = 75) { ## griglia 25*25
  ...
  x2 = seq(from = rang[1,2], to = rang[2,2], length = n) ##distribuzione omogenea
...
plot(xgrid, col = c("red","blue")[as.numeric(ygrid)], pch = 20, cex=.2) ##mi converte la variabile fattore in numero
points(x, pch = 17, col=y+3, cex=1) ##metto i punti originari
