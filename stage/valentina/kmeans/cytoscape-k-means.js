;(function(){ 'use strict';
  ...
  var nodes_from_data = function(data){
    var nodes=[]
    for(var i=0;i<data.length;i++)
      nodes.push({id:i,p:data[i]})      
    return nodes;
  }
  ...
  var randomCentroids = function( nodes, k, attributes ) {
    ...
    for ( var i = 0; i < ndim; i++ ) {
      ...
      min[i] = find_min(nodes,attributes[i])
      //max[i] = nodes.max( attributes[i] ).value;
      max[i] = find_max(nodes,attributes[i])
    }
  ...
  var kMeans = function(data, options ){
    //var cy    = this.cy();
    var nodes = nodes_from_data(data)
    var node  = null;
    var opts  = {};
  ...
  var kMedoids = function( data, options ) {
    //var cy    = this.cy();
    var nodes = nodes_from_data(data);
    var node  = null;
    var opts  = {};
  ...
  var fuzzyCMeans = function( data, options ) {
    //var cy    = this.cy();
    var nodes = nodes_from_data(data); //this.nodes();
    var node  = null;
    var opts  = {};
    ...
    clusters = assign( nodes, U, opts, null /*cy*/ );
  ...
  // prova tremenda ... aggiungo a ML
  ML.fromCytoscape={kMeans:kMeans,kMedoids:kMedoids,fuzzyCMeans:fuzzyCMeans}
